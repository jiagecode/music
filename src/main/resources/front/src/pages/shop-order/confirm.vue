<template>
  <div :style='{"width":"100%","padding":"40px 7%","margin":"0 auto","position":"relative","background":"none"}'>
    <div class="section-title" :style='{"margin":"20px 7%","color":"#333","borderRadius":"20px","textAlign":"center","background":"#ceeff3","width":"86%","fontSize":"16px","lineHeight":"40px"}'>确认下单</div>
                ...
                this.$http.get(`${item.tablename}/info/${item.goodid}`).then(res => {
                    ...
                    this.$http.post(`${item.tablename}/update`, data).then(res => {
                        ...
                        this.$http.post('orders/add', order).then(res => {
                            // 减少余额，更新订单状态
                            // 判断余额是否充足
                            if (Number(this.user.jf) < Number(item.price * item.buynumber)) {
                                this.$message({
                                    message: '积分不足',
                                    type: 'error',
                                    duration: 1500

...
<div :style='{"width":"100%","padding":"40px 7%","margin":"0 auto","position":"relative","background":"none"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0","background":"none"}' label="歌曲下载" prop="gequxiazai">
            <file-upload
            tip="点击上传歌曲下载"
            action="file/upload"
            :limit="1"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('goumaituisong/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('goumaituisong/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('goumaituisong/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
      ...
      gequxiazaiUploadChange(fileUrls) {
          this.ruleForm.gequxiazai = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>

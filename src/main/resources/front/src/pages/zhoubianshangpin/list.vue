...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"border":"0 outset #bcdbdf","padding":"30px 20px","margin":"20px 7% 0","alignItems":"center","flexWrap":"wrap","background":"#fff","borderWidth":"1px 0 2px","display":"flex","width":"86%","height":"auto"}'>
      ...
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
        ...
        <el-select v-model="formSearch.shangpinpinpai" placeholder="请选择商品品牌" :clearable="true">
          <el-option v-for="(item, index) in shangpinpinpaiOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	  ...
	  <el-button v-if="isAuth('zhoubianshangpin','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 10px","margin":"0 4px 0 0","color":"#fff","minWidth":"90px","outline":"none","borderRadius":"30px","background":"#f7aa00","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/zhoubianshangpinAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"padding":"40px 0 20px","margin":"40px 7% 0","background":"none","flex":"1","width":"100%","minWidth":"850px","order":"4"}'>
		...
		<div class="list2 index-pv1" :style='{"padding":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"cursor":"pointer","padding":"10px","margin":"0 0 100px","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230206/de4c50b5282f45f8a59707bce3185db8.png) no-repeat left bottom / 20%,url(http://codegen.caihongy.cn/20230206/5e80378b411c4449a860d66e35c5c969.png) no-repeat right top / 20%","display":"flex","width":"49%","fontSize":"0","position":"relative","justifyContent":"space-between","height":"240px"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"width":"52%","padding":"20px","objectFit":"contain","display":"inline-block","height":"100%"}' v-if="item.tupian && item.tupian.substr(0,4)=='http'" :src="item.tupian" class="image" />
				<img :style='{"width":"52%","padding":"20px","objectFit":"contain","display":"inline-block","height":"100%"}' v-else :src="baseUrl + (item.tupian?item.tupian.split(',')[0]:'')" class="image" />
				<div class="item-info" :style='{"padding":"10px","overflow":"hidden","top":"60px","alignItems":"flex-start","borderRadius":"8px","flexWrap":"wrap","display":"flex","width":"44%","position":"absolute","right":"30px","justifyContent":"center","height":"90%"}'>
					<div :style='{"border":"0 dashed #bcdbdf","padding":"0 10px","margin":"0 10px 0 0","color":"#333","textAlign":"center","borderWidth":"0 1px 0 0","width":"40px","lineHeight":"1.2","fontSize":"14px"}' class="name ">{{item.shangpinmingcheng}}</div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/shangpinpinpai/shangpinpinpai').then(res => {
        if (res.data.code == 0) {
          this.shangpinpinpaiOptions = res.data.data;
        }
      });
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        this.$http.get('zhoubianshangpin/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/zhoubianshangpinDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>

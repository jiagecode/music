<template>
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px 30px 30px 30px","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20220730/cb552a530d5f4a9ab3f49b479dc1140f.png) center center/cover no-repeat fixed","flexDirection":"row-reverse","display":"flex","height":"auto"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="商品品牌" prop="shangpinpinpai">
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shangpinpinpai/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shangpinpinpaiCrossAddOrUpdateFlag = false;
						 this.parent.search();

		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"width":"10%","padding":"0 20px","margin":"0 0 20px 0","background":"#fff"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"margin":"20px 0","alignItems":"center","flexDirection":"column","display":"flex"}'>
					<el-button :style='{"border":"1px solid ","cursor":"pointer","padding":"0 24px","margin":"0 0 6px","outline":"none","color":"#857EF0","borderRadius":"0","background":"#fff","borderImage":"linear-gradient(180deg, rgba(133.00000727176666, 126.00000008940697, 240.00000089406967, 1), rgba(133.00000727176666, 126.00000008940697, 240.00000089406967, 0.10000000149011612)) 1 1","width":"100%","fontSize":"14px","height":"40px"}' v-if="isAuth('discusszhoubianshangpin','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"1px solid","cursor":"pointer","padding":"0 24px","margin":"0 0 6px","outline":"none","color":"#857EF0","borderRadius":"0","background":"#fff","borderImage":"linear-gradient(180deg, rgba(133.00000727176666, 126.00000008940697, 240.00000089406967, 1), rgba(133.00000727176666, 126.00000008940697, 240.00000089406967, 0.10000000149011612)) 1 1","width":"100%","fontSize":"14px","height":"40px"}' v-if="isAuth('discusszhoubianshangpin','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"1px solid","cursor":"pointer","padding":"0 24px","margin":"0 10px 6px 0","outline":"none","color":"#857EF0","borderRadius":"0","background":"#fff","borderImage":"linear-gradient(180deg, rgba(133.00000727176666, 126.00000008940697, 240.00000089406967, 1), rgba(133.00000727176666, 126.00000008940697, 240.00000089406967, 0.10000000149011612)) 1 1","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('discusszhoubianshangpin','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>


							<el-button :style='{"border":"1px solid","cursor":"pointer","padding":"0 24px","margin":"0 10px 6px 0","outline":"none","color":"#857EF0","borderRadius":"0","background":"#fff","borderImage":"linear-gradient(180deg, rgba(133.00000727176666, 126.00000008940697, 240.00000089406967, 1), rgba(133.00000727176666, 126.00000008940697, 240.00000089406967, 0.10000000149011612)) 1 1","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('discusszhoubianshangpin','查看评论')" type="primary" size="mini" @click="disscussListHandler(scope.row.id)">查看评论</el-button>


							<el-button :style='{"border":"1px solid","cursor":"pointer","padding":"0 24px","margin":"0 10px 6px 0","outline":"none","color":"#857EF0","borderRadius":"0","background":"#fff","borderImage":"linear-gradient(180deg, rgba(133.00000727176666, 126.00000008940697, 240.00000089406967, 1), rgba(133.00000727176666, 126.00000008940697, 240.00000089406967, 0.10000000149011612)) 1 1","width":"auto","fontSize":"14px","height":"32px"}' type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">回复</el-button>

							<el-button :style='{"border":"1px solid","cursor":"pointer","padding":"0 24px","margin":"0 10px 6px 0","outline":"none","color":"#857EF0","borderRadius":"0","background":"#fff","borderImage":"linear-gradient(180deg, rgba(133.00000727176666, 126.00000008940697, 240.00000089406967, 1), rgba(133.00000727176666, 126.00000008940697, 240.00000089406967, 0.10000000149011612)) 1 1","width":"auto","fontSize":"14px","height":"32px"}' v-if="" type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.$http({
        url: "discusszhoubianshangpin/page",
        method: "get",
        params: params
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.$router.push({path:'/discussdiscusszhoubianshangpin',query:{refid:id}});
    },
    // 下载
    download(file){
      window.open(`${file}`)
    ...
    deleteHandler(id) {
      ...
      this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
        ...
        this.$http({
          url: "discusszhoubianshangpin/delete",
          method: "post",
          data: ids
        }).then(({ data }) => {
          if (data && data.code === 0) {

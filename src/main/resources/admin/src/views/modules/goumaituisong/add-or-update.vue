	...
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px 30px 30px 30px","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20220730/cb552a530d5f4a9ab3f49b479dc1140f.png) center center/cover no-repeat fixed","flexDirection":"row-reverse","display":"flex","height":"auto"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="歌曲分类" prop="gequfenlei">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.gequxiazai" label="歌曲下载" prop="gequxiazai">
					<file-upload
						tip="点击上传歌曲下载"
						action="file/upload"
						:limit="1"
						:multiple="true"
						:fileUrls="ruleForm.gequxiazai?ruleForm.gequxiazai:''"
						@change="gequxiazaiUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="ruleForm.gequxiazai" label="歌曲下载" prop="gequxiazai">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 25px","margin":"0 20px 0 0","outline":"none","color":"rgba(255, 255, 255, 1)","borderRadius":"4px","background":"#857EF0","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' type="text" size="small" @click="download($base.url+ruleForm.gequxiazai)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="!ruleForm.gequxiazai" label="歌曲下载" prop="gequxiazai">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="用户" prop="yonghu">
					<el-input v-model="ruleForm.yonghu" placeholder="用户" clearable  :readonly="ro.yonghu"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="用户" prop="yonghu">
					<el-input v-model="ruleForm.yonghu" placeholder="用户" readonly></el-input>
				</el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='gequxiazai'){
							this.ruleForm.gequxiazai = obj[o];
							this.ro.gequxiazai = true;
							continue;
						}
						if(o=='yonghu'){
	...
	if(this.ruleForm.gequxiazai!=null) {
		this.ruleForm.gequxiazai = this.ruleForm.gequxiazai.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `goumaituisong/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.goumaituisongCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    gequxiazaiUploadChange(fileUrls) {
	    this.ruleForm.gequxiazai = fileUrls;
    },
  }
};
</script>
